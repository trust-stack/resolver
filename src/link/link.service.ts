...
export async function createLink(dto: CreateLinkDto): Promise<LinkDto> {
...
export async function updateLink(id: string, dto: UpdateLinkDto): Promise<LinkDto | null> {
...
export async function listLinks(query: ListLinksQueryDto): Promise<PaginatedLinksDto> {
  const { linksRepository } = getRequestContext();
